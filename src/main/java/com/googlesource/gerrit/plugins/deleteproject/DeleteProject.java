...
class DeleteProject implements RestModifyView<ProjectResource, Input> {
  ...
  private final Provider<ReplicatedEventsCoordinator> providedCoordinator;
  ...
  DeleteProject(
      ...
      Provider<ReplicatedEventsCoordinator> eventsCoordinator) {
    ...
    this.providedCoordinator = eventsCoordinator;
  }

  //Lazy initialize coordinator
  public ReplicatedEventsCoordinator getReplicatedEventsCoordinator(){
    if(replicatedEventsCoordinator == null){
      replicatedEventsCoordinator = providedCoordinator.get();
    }
  ...
  public void doDelete(ProjectResource rsrc, Input input) throws IOException, RestApiException {
    ...
    if (!replicatedRepo) {
      ...
      if (getReplicatedEventsCoordinator().isReplicationEnabled()) {
  ...
  public static String getProperty(File appProps, String propertyName) throws IOException {
    ...
    try {
      ...
      return props.getProperty(propertyName);
        ...
        throw new IOException("Could not read " + appProps.getAbsolutePath(), e);
    } finally {
      if (input != null) {
        try {
          input.close();
  ...
  public void archiveAndRemoveRepo(Project project, String uuid) throws IOException {
    ...
    if (port != null && !port.isEmpty()) {
      try {
        ...
        ee.initCause(e);
        log.atSevere().withCause(ee).log("Error with deleting repo: %s", projectPath);
        throw ee;
      }
    }
  }
  ...
  private HttpRequestBuilder getRequestBuilder(String endpoint, final HttpRequestBuilder.WebRequestType type) {
    final GitMsApplicationProperties appProps = getReplicatedEventsCoordinator()
        .getReplicatedConfiguration().getGitMsApplicationProperties();
   ...
   */
  private void makeGitMSDeleteRequest(String uuid, String repoPath) throws IOException, URISyntaxException {

    HttpRequestBuilder deleteRequestBuilder = getRequestBuilder(requestEndpoint, DELETE);

    // Check the parameters for the request are valid.
    ...
    if (response.statusCode != 202 && response.statusCode != 200) {
      ...
      throw new IOException(String.format("Failure to delete the git repository on the GitMS Replicator, " +
          "response code: %s, Replicator response was: %s", response.statusCode, response.response));
    }
  }
  ...
  private ApiResponse makeGitMSNodeMembershipRequest(String repoPath)
    ...
    HttpRequestBuilder membershipRequestBuilder = getRequestBuilder(requestEndpoint, GET);
    // Request parameters appended onto the delete request of the form
    ...
    if (response.statusCode != 202 && response.statusCode != 200) {
      ...
      throw new IOException(String.format("Failure get a the nodeIds for the repository membership: %s %s", response, response.response));
    }
    return response;
  }
  ...
  public void deleteReplicatedRepo(ProjectResource rsrc, Input input, Project project, boolean preserve)
      throws IOException, RestApiException, InvalidParameterException {
    ...
    try {
      if (!preserve || !cfg.projectOnPreserveHidden()) {
        ...
        try {
          ...
          fsHandler.deleteFromCache(project.getNameKey());
        } catch (RepositoryNotFoundException e) {
          log.atSevere().withCause(e).log("Could not find the project %s", project.getName());
          throw e;
        }
      ...
      final String localNodeId = Objects.requireNonNull(replicatedEventsCoordinator.getThisNodeIdentity());
      ...
      if(isSingleNodeMembership){
        ...
        getReplicatedEventsCoordinator()
            .getReplicatedOutgoingProjectEventsFeed().deleteProjectSingleNodeGroup(project, preserve, uuid);
      } else {
        ...
        log.atInfo().log("About to call ReplicatedProjectManager.replicateProjectDeletion(): %s, %s",
            project.getName(), preserve);
        getReplicatedEventsCoordinator()
            .getReplicatedOutgoingProjectEventsFeed().replicateProjectDeletion(project.getName(), preserve, uuid);
      }
  ...
  public void deleteProjectChanges(boolean preserve, List<Change.Id> changeIds,
    ...
    getReplicatedEventsCoordinator()
        .getReplicatedIncomingIndexEventProcessor().deleteChangesByChangeId(changeIds);
    ...
    if(!isSingleNodeMembership) {
      String uuid = UUID.randomUUID().toString();
      getReplicatedEventsCoordinator()
          .getReplicatedOutgoingProjectEventsFeed().replicateProjectChangeDeletion(project, preserve, changeIds, uuid);
    }
  }
}
