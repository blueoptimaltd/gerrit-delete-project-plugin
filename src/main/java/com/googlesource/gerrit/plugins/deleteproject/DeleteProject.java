...
class DeleteProject implements RestModifyView<ProjectResource, Input> {
  ...
  public void deleteReplicatedRepo(ProjectResource rsrc, Input input, Project project, boolean preserve)
    ...
    try {
      ...
      getReplicatedEventsCoordinator().getReplicatedOutgoingProjectEventsFeed().replicateProjectDeletion(project.getName(), preserve, uuid);
  ...
  public void deleteProjectChanges(boolean preserve, List<ChangeData> changes, Project project) throws IOException{
    ...
    getReplicatedEventsCoordinator().getReplicatedIncomingIndexEventProcessor().deleteChanges(changes);

    //Replicate changesToBeDeleted across the nodes
    String uuid = UUID.randomUUID().toString();
    getReplicatedEventsCoordinator().getReplicatedOutgoingProjectEventsFeed().
        replicateProjectChangeDeletion(project, preserve, changeIds, uuid);
  }
}
