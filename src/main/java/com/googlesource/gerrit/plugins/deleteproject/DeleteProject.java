  ...
  public void archiveAndRemoveRepo(Project project, String uuid) throws IOException {
    ...
    if (port != null && !port.isEmpty()) {
      try {
        ...
        IOException ee = new IOException("Error with deleting repo: " + e);
        ee.initCause(e);
        log.atSevere().withCause(ee).log("Error with deleting repo: %s", projectPath.toString());
        throw ee;
      }
    }
  }
    ...
    try {
      if (!preserve || !cfg.projectOnPreserveHidden()) {
        ...
        try {
          ...
          log.atSevere().withCause(e).log("Could not delete the project %s", project.getName());
          throw e;
        }
